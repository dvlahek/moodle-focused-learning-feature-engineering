...
def normalize_logs(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    logs["timestamp"] = pd.to_datetime(
        logs["timestamp"], errors="coerce", format="mixed", dayfirst=True
    )
...
def normalize_grades(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only explicitly supported assessment/course-record fields.

    This explicit whitelist prevents unrelated grade-book columns from silently
    entering the predictive feature matrix.
    """
    ...
    grades["kolokvij1"] = pd.to_numeric(
        grades["kolokvij1"].astype(str).str.replace(",", ".", regex=False),
        errors="coerce",
    )

    keep = required.copy()
    if "Vjezbe" in grades.columns:
        grades["Vjezbe"] = pd.to_numeric(
            grades["Vjezbe"].astype(str).str.replace(",", ".", regex=False),
            errors="coerce",
        )
        keep.append("Vjezbe")

    return (
        grades.dropna(subset=required)[keep]
...
def session_gap_statistics(sessions: list[list[pd.Timestamp]]) -> tuple[float, float, float]:
    ...
    gaps = [
        round(
            (sessions[index][0] - sessions[index - 1][-1]).total_seconds() / 3600,
            1,
        )
...
def material_events(student_logs: pd.DataFrame) -> pd.DataFrame:
    """Select first-part learning-material PDF events.

    The protected study export used Croatian contexts of the form
    ``Datoteka: 1`` ... ``Datoteka: 8``. The synthetic package uses the
    equivalent English ``Learning material 1`` ... ``Learning material 8``
    labels. In both cases the resource must be a PDF.
    """
    context = student_logs["context"].fillna("")
    croatian = context.str.contains(
        r"Datoteka:\s*(?:[1-8])(?:\D|$)", regex=True, case=False, na=False
    )
    english = context.str.contains(
        r"Learning\s+material\s*(?:[1-8])(?:\D|$)", regex=True, case=False, na=False
    )
    pdf = context.str.contains("pdf", regex=False, case=False, na=False)
    return student_logs.loc[(croatian | english) & pdf].copy()


def estimate_focused_start(
    ...
    *,
    ...
    times = times[times <= pd.Timestamp(exam)].reset_index(drop=True)
    if times.empty:
        raise ValueError(
            "Cannot estimate a focused-learning window without pre-assessment material activity"
        )

    duration = pd.Timedelta(days=int(window_days))
    best_count = -1
    best_window_start: pd.Timestamp | None = None

    # Study-faithful tie behavior: the first candidate reaching the maximum is
    # retained because the timestamps are sorted chronologically.
    for start in times:
        count = int(((times >= start) & (times < start + duration)).sum())
        if count > best_count:
            best_count = count
            best_window_start = pd.Timestamp(start)

    if best_window_start is None:
        raise ValueError("Focused-learning window could not be estimated")

    selected = times[
        (times >= best_window_start) & (times < best_window_start + duration)
    ]
    ...
    return pd.Timestamp(focused_start), best_window_start, int(best_count)


def homework_features(student_logs: pd.DataFrame) -> tuple[float, int]:
    component = student_logs["component"].str.lower()
    assignment_mask = component.str.contains(r"assignment|zada", regex=True, na=False)
    ...
    for _, group in assignments.groupby("context"):
        ...
        names = ordered["event_name"].str.lower()
        submitted = ordered[
            names.str.contains(r"assignment submitted|zadaća je predana|zadaca je predana", regex=True, na=False)
        ]
        submit_time = (
            submitted["timestamp"].iloc[0]
            if not submitted.empty
            else ordered["timestamp"].iloc[-1]
        )
        total_hours += max(
            0.0,
            (submit_time - ordered["timestamp"].iloc[0]).total_seconds() / 3600,
        )
        ...
        status_views_after_submission += int(
            ...
            .str.lower()
            .str.contains(
                r"submission status viewed|pregledano je stanje predane zadaće|pregledano je stanje predane zadace",
                regex=True,
                na=False,
            )
            .sum()
...
def extract_behavioral_features(
    ...
    *,
    ...
    before_exam = student_logs[student_logs["timestamp"] < pd.Timestamp(exam)].copy()
    materials = material_events(before_exam)
    if materials.empty:
        raise ValueError("No learning-material events")

    focused_start, best_window_start, best_window_count = estimate_focused_start(
        materials["timestamp"], exam, window_days=window_days
    )
    ...
    focused_times = [
        timestamp
        for timestamp in material_times
        if focused_start <= timestamp <= pd.Timestamp(exam)
    ]
    ...
    days_before = max(
        (pd.Timestamp(exam) - focused_start).total_seconds() / 86400,
        1 / 24,
    )
    ...
    quiz_mask = before_exam["component"].str.lower().str.contains(
        r"quiz|test", regex=True, na=False
    )
    quizzes = before_exam.loc[quiz_mask]
    quiz_total = len(quizzes)
    quiz_focused = (
        int((quizzes["timestamp"] >= focused_start).sum()) if quiz_total else 0
    )
    quiz_earliest_h = (
        float(
            (pd.Timestamp(exam) - quizzes["timestamp"].min()).total_seconds() / 3600
        )
        if quiz_total
        else 0.0
    )
    ...
    row: dict[str, float | pd.Timestamp] = {
        ...
        "start_learning_before_exam_d": float(days_before),
        "start_learning_before_exam_h": float(hours_before),
    ...
    for part in DAY_PARTS:
        ...
        row[f"activity_start_learning_before_exam_{part}"] = float(
            focused_parts[part]
        )
...
def load_weather(path: str | Path) -> pd.DataFrame:
    ...
    # Accept both ISO and common Croatian day/month/year representations.
    parsed = pd.to_datetime(weather["date"], errors="coerce", dayfirst=True)
    weather["date"] = parsed.dt.normalize()
...
def add_environment_and_interactions(
    row: dict,
    ...
    for column in WEATHER_COLUMNS:
        ...
        row[f"activities_x_{weather_name}"] = (
            row["activities_during_learning"] * mean_value
        )
        row[f"activity_per_day_x_{weather_name}"] = (
            row["activity_per_learning_day"] * mean_value
        )
        row[f"sessions_x_{weather_name}"] = row["session_Sum"] * mean_value
        row[f"regularity_x_{weather_name}"] = (
            row["regularity_inverse_mean_periodicity"] * mean_value
        )
    return row
...
def build_year_dataset(
    ...
    *,
    ...
    for student_id, group in logs.groupby("student_id"):
        try:
            features = extract_behavioral_features(
                group, exam, window_days=window_days
            )
        except ValueError:
            continue
    ...
    if frame.empty:
        raise ValueError(f"No usable students were extracted for year {year}")

    frame = frame.merge(grades, on="student_id", how="inner", validate="one_to_one")
    frame = frame.drop(
        columns=["focused_start_timestamp", "best_window_start_timestamp"],
        errors="ignore",
    )
    return coerce_numeric_frame(frame)
